...
**Phase:** 5 — Skill host + `brief` — **built, all DoD checks self-run PASS, not yet merged**
**Status:** The skill host (`core/skills/`) — manifest loading with error
isolation, two-stage routing (lane classifier → embedding match →
disambiguation), `SkillContext`, per-skill namespaced storage, an ESLint
rule blocking executor imports, and `make new-skill` — plus the `brief`
skill. All four DoD numbers measured: a real "good morning" produces a
spoken brief drawn from real memory (live, real Ollama embeddings + NIM),
a deliberately broken skill is disabled without taking core down (live),
intent routing is **100%** on a 15-case live benchmark (bar: ≥90%), and
`make new-skill` timed at under two minutes end to end (bar: <30min) —
see the Phase 5 log below for the real bugs this phase's own live testing
found along the way (four of them, each fixed).
